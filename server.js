...
// ---------- full catalogue from the live feed (cached) ----------
...
function toneOf(title){
  ...
  if(GREEN.some(w=>t.includes(w))) return 'green';
  if(WARM.some(w=>t.includes(w))) return 'warm';
  if(COOL.some(w=>t.includes(w))) return 'cool';
  if(NEUTRAL.some(w=>t.includes(w))) return 'neutral';
  return 'neutral';
}
function motifOf(material){
  const m=(material||'').toLowerCase();
  if(m.includes('herringbone')) return 'herringbone';
  if(m.includes('stripe')) return 'stripe';
  if(m.includes('damask')) return 'damask';
  if(m.includes('floral')) return 'floral';
  if(m.includes('trellis')) return 'trellis';
...
async function loadCatalogue(){
  ...
  for(const chunk of items){
    ...
    out.push({
      id, name: title.replace(/\s*Fabric\s*$/i,'').trim(), url,
      price: Math.round(parseFloat(g('g:price'))||0),
      tone: toneOf(title), motif: motifOf(g('g:material')),
      outlet: (g('g:custom_label_1')||'').trim().toLowerCase()==='outlet',
    });
  }
  if(out.length){ CATALOGUE=out; CAT_AT=Date.now(); }
...
// ---------- analyse the room, choose 4 from the whole catalogue ----------
app.post('/api/analyse', async (req, res) => {
  try {
    ...
    const prompt =
      ...
      '{"style":"short phrase describing the room","wall":"the wall colour in plain words","light":"the light in the room, e.g. bright natural daylight or soft north-facing light or warm evening light","palette":["#hex","#hex","#hex"],' +
    ...
    let txt = textOf(response).trim(); let parsed;
